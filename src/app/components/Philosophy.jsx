			...
			<div className="max-w-7xl mx-auto relative z-10">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
					{principles.map((principle, i) => (
						<div
							key={i}
							className="group relative p-5 sm:p-6 rounded-2xl bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-200/80 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 hover:shadow-xl hover:shadow-slate-500/10"
							ref={el => (principlesRef.current[i] = el)}
						>
							<div className="text-5xl sm:text-6xl font-black text-slate-200 dark:text-slate-700 mb-3 sm:mb-4 select-none group-hover:text-slate-300 dark:group-hover:text-slate-600 transition-all duration-300">
								{principle.num}
							</div>
							<h3 className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-white mb-2 sm:mb-3">
								{principle.title}
							</h3>
							<p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{principle.desc}</p>
						</div>
					))}
				</div>
			</div>
